...
export default function UserPage() {
  ...
  if (loading) return <p className="text-center mt-20 text-laranja font-sans font-bold">Carregando usuário...</p>;
  if (!usuario) return <p className="text-center mt-20 text-laranja font-sans font-bold">Usuário não encontrado.</p>;
    ...
    <main className="min-h-screen text-text bg-back pb-16">
        ...
        {Dono && (
          <div>
            <button 
              ...
              className="absolute top-5 flex right-20 bg-back border border-laranja text-laranja px-20 py-2 rounded-full hover:bg-laranja hover:text-white transition hover:cursor-pointer font-sans tracking-wider"
            >
              Editar Perfil
            </button>
          </div>
      ...
      <div className="w-full max-w-5xl font-sans mx-auto mt-[200px] px-4">
        <div className="flex items-center justify-between w-full mb-4">
          <h3 className="text-text font-sans font-bold text-2xl md:text-4xl">
            <Link href="/categories" className="hover:text-laranja transition-colors">
              Produtos
            </Link>
            <span className="text-xl md:text-xl font-normal text-gray-500 block md:inline md:ml-2">
              de <strong className="text-laranja">{usuario.username}</strong>
            </span> 
          </h3>

          {produtos.length > 0 && (
            <div
              className="px-3 py-1 flex items-center hover:underline justify-center text-laranja font-bold text-sx rounded-full hover:brightness-90 hover:cursor-pointer transition"
          ...
          <Carrossel>
            {produtos.length > 0 ? (
              produtos.map((produto) => (
                <Link key={produto.id} href={`/product/${produto.id}`} className="block h-full hover:brightness-90">
      ...
      <div className="w-full max-w-5xl mx-auto mt-12 px-4">
        <div className="flex items-center justify-between w-full mb-4">
          <h3 className="text-text font-sans font-bold text-2xl md:text-4xl">
            <Link href="/categories-stores" className="hover:text-laranja transition-colors">
              Lojas
            </Link>
            <span className="text-xl md:text-xl font-normal text-gray-500 block md:inline md:ml-2">
              de <strong className="text-laranja">{usuario.username}</strong>
        ...
        <div className="flex rounded-3xl font-sans gap-6">
          <Carrossel>
            {lojas.length > 0 ? (
              lojas.map((loja) => (
                <Link key={loja.id} href={`/store/${loja.id}`} className="block">
                  <div className="shrink-0 min-w-[400px] gap-2 bg-card text-text shadow rounded-4xl p-4 h-40 flex items-center justify-center text-gray-800 hover:scale-105 transition-transform font-semibold">
      ...
      <div className="w-full max-w-5xl mx-auto mt-12 px-4">
        ...
        <h3 className="text-text font-sans font-bold text-2xl my-4 md:text-md">
            Avaliações
            <span className="text-md md:text-base font-normal text-gray-500 block md:inline md:ml-2">
            em <strong className="text-laranja">Lojas</strong>
          </span> 
        </h3>
          ...
          <Carrossel>
            {avaliacoes?.store_ratings?.length ? (
              avaliacoes.store_ratings.map((a) => (
                <Link key={`store-${a.id}`} href={`/rating/store/${a.id}`} className="block h-full">
                  <div className="min-w-[400px] bg-card shadow rounded-4xl p-4 py-8 flex flex-col justify-between hover:brightness-90 transition h-full">
                    <div className="flex justify-between">
                      <div className="flex items-center gap-2">
                        ...
                        />
                        <div>
                          <p className="font-semibold text-text">{usuario?.name}</p>
                    ...
                    <p className="mt-2 text-text line-clamp-2">{a.comment || "Sem comentário"}</p>
                    <p className="text-sm mt-1 text-gray-500 font-medium">Loja: {a.store?.name}</p>
                  </div>
                </Link>
              ))
            ) : (
        ...
        <h3 className="text-text font-sans font-bold text-2xl my-4 md:text-md">
            Avaliações
            <span className="text-md md:text-base font-normal text-gray-500 block md:inline md:ml-2">
            em <strong className="text-laranja">Produtos</strong>
          </span> 
        </h3>
        <div className="flex relative rounded-3xl font-sans gap-6">
          <Carrossel>
            {avaliacoes?.product_ratings?.length ? (
              avaliacoes.product_ratings.map((a) => (
                <Link key={`product-${a.id}`} href={`/rating/product/${a.id}`} className="block h-full">
                  <div className="min-w-[400px] bg-card shadow rounded-4xl p-4 py-8 flex flex-col justify-between hover:brightness-90 transition h-full">
                    <div className="flex justify-between">
                    ...
                    <p className="mt-2 text-text line-clamp-2">{a.comment || "Sem comentário"}</p>
                    <p className="text-sm mt-1 text-gray-500 font-medium">Produto: {a.product?.name}</p>
                  </div>
                </Link>
              ))
            ) : (
